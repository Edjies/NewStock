...
def select(stock_list, x_position=-1, min_item=120):
    ...
    for stock in stock_list:
        ...
        sma5_day, sma10_day, sma20_day, sma60_day = StockIndicator.sma(kline_day, 5, 10, 20, 60)
        if stock.stock_code == '300146':
            print(stock.stock_code)

        if sma5[x_position] > sma20[x_position] or sma5[x_position] > sma60[x_position]:
            if sma5_day[x_position] > sma20_day[x_position] or sma5_day[x_position] > sma60_day[x_position]:
                print(stock)
                result.append(stock)
    return result
