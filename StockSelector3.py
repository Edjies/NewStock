...
@filtrate_high_price
@filtrate_stop_trade
def select(stock_list, kline_type=StockConfig.kline_type_week, x_position=-1, min_chg=-100, max_chg=100, min_vb = 15, max_vb=100, min_item=120):
    """
    根据 振幅区间 和 涨幅区间排序
    :param stock_list:
    :param kline_type:
    ...
    for stock in stock_list:
        ...
        if max_chg > chg[x_position] > min_chg and max_vb > vb[x_position] > min_vb:
            # 过滤掉单边上涨
            if not StockAlgrithm.sumOfSubArray(chg[-10:])[0] > 14:
                print(stock)
                result.append(stock)
    return result
...
if __name__ == '__main__':
    ...
    for x in range(-5, 0):
        ...
        stock_list = select(StockIO.get_stock('level_1'), x_position=x, kline_type=StockConfig.kline_type_week,
                 min_chg=-100, max_chg=100, min_vb=14, max_vb=100)
        print(stock_list)
